...
@patch("api.sheets.sheets_repo.get_main_sheet")
class TestReadRows:
    def test_returns_all_records(self, mock_sheet):
        records = [{"id": "1", "date": "2026-01-01", "weight_kg": "80"}]
        mock_sheet.return_value.worksheet.return_value.get_all_records.return_value = records

        result = sheets_repo.read_rows("WeightLogs")

        assert result == records

    def test_returns_empty_list_when_no_data(self, mock_sheet):
        mock_sheet.return_value.worksheet.return_value.get_all_records.return_value = []
...
@patch("api.sheets.sheets_repo.get_main_sheet")
class TestAppendRow:
    def test_appends_values_in_header_order(self, mock_sheet):
    ...
    def test_missing_keys_fill_with_empty_string(self, mock_sheet):
        ws = _make_ws(["id", "date", "weight_kg"], [])
        mock_sheet.return_value.worksheet.return_value = ws
...
@patch("api.sheets.sheets_repo.get_main_sheet")
class TestUpdateRow:
    def test_updates_correct_range(self, mock_sheet):
        ws = _make_ws(["id", "date", "weight_kg"], [])
        mock_sheet.return_value.worksheet.return_value = ws
...
@patch("api.sheets.sheets_repo.get_main_sheet")
class TestFindRow:
    def test_returns_row_index_and_record_when_found(self, mock_sheet):
        records = [
    ...
    def test_returns_none_when_not_found(self, mock_sheet):
    ...
    def test_returns_first_match_when_duplicates(self, mock_sheet):
        records = [
            {"id": "1", "date": "2026-01-01"},
